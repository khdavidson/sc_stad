...
sj.plot <- sj.esc.raw %>% 
  ...
  pivot_longer(cols=c("Max Estimate": "Natural spawners"), names_to = "CK_est", values_to = "n") %>% 
...
ggplot() +
  ...
  geom_segment(aes(x=1999,
                   xend=sj.plot %>% 
                     filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook", CK_est=="Broodstock removals", !is.na(n)) %>% 
                     summarize(max=max(`Analysis Year`))%>%
                     pull(max),
                   y=sj.plot %>% 
                     filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook",CK_est%in%c("Natural spawners", "Broodstock removals"), 
                            `Analysis Year`>=1998) %>% 
    ...
    geom_segment(aes(x=sj.plot %>% 
                     filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook", CK_est=="Broodstock removals", is.na(n)) %>% 
                     summarize(min=min(`Analysis Year`))%>%
                     pull(min),
                   xend=1994,
                   y=sj.plot %>% 
                     filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook",CK_est%in%c("Natural spawners", "Broodstock removals"), 
  ...
  #geom_hline(aes(yintercept=sj.plot %>% 
  #                 filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook", CK_est%in%c("Natural spawners", "Broodstock removals")) %>%
  #                 group_by(`Analysis Year`) %>% 
  #                 summarize(sum=sum(n, na.rm=T)) %>% 
  #                 summarize(mean=mean(sum), sd(sum)) %>%
  #                 pull(mean)),
  #           colour="red", linetype="solid", size=1) +
  geom_bar(data=sj.plot %>% filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook", CK_est%in%c("Natural spawners", "Broodstock removals")), 
           aes(x=as.numeric(`Analysis Year`), y=as.numeric(n), group=CK_est, fill=CK_est),
  ...
  labs(x="Return Year", y="Number of San Juan Chinook", caption="Fig 1. Total natural spawners (jack+adult) and broodstock removals for San Juan Chinook. Red lines indicate \naverage escapements (spawenrs+brood) before and after hatchery activities began. Data from NuSEDS") +
  theme_bw() +
...
# JOIN:
sj.rel.all <- full_join(sj.rel.mrp.sum, sj.rel.sep.sum) %>%
  ...
  select(FACILITY_NAME, `Release Site Name`, BROOD_YEAR, CWT_AD, AD_only, unmarked, total_release,
         `CWT_AD %`, `AD_only %`, `unmarked %`, source)





# MRP Releases that were recovered -----------------------
# How many
sj.rel.rcvy %>% 
...
Fig 1/Fig 3 shows brood collections over time (as documented in NuSEDS). Hatchery releases from SEP are documented back to 1995, but note that MRP also contains San Juan CWT records dating back to BY 1979. From 2013-2017 all seapean releases were marked, but the seapen has not been used in recent years (although hopes to be used for BY 2022/RY 2023). The majority of San Juan chinook releases are unmarked. 

*Table 2. Hatchery releases and marking information for San Juan and tributary Chinook. Data from SEP. It is assumed all broodstock was collected from the San Juan River, with different release strategy locations indicated by "Release Site Name" (Port Renfrew = juveniles reared in a sea netpen).*
```{r echo=F, warning=F, message=F}
# Just Releases ----------------------- 
sj.rel.all %>%
filter(!grepl("Harris", `Release Site Name`)) %>%
  group_by(BROOD_YEAR, FACILITY_NAME) %>% 
  summarize(CWT_AD=unique(CWT_AD), AD_only=sum(AD_only), unmarked=unique(unmarked), total_release=unique(total_release), `CWT_AD %` = unique(`CWT_AD %`),
            `AD_only %`=unique(`AD_only %`), `unmarked %`=unique(`unmarked %`)) %>% 
...
```{r message=F, warning=F, echo=F}
ggplot() +
  geom_bar(data=sj.rel.all, aes(x=as.numeric(BROOD_YEAR), y=total_release/200, fill=`Release Site Name`), stat="identity", alpha=0.3) +
  geom_line(data=sj.plot%>%filter(`Waterbody Name`=="SAN JUAN RIVER", Species=="Chinook", CK_est%in%c("Broodstock removals","Natural spawners")), 
            aes(x=as.numeric(`Analysis Year`), y=as.numeric(n), group=CK_est, colour=CK_est), size=1.2, alpha=0.7) +
  ...
  labs(x="Brood year (RY is +1)", y="Number of Chinook", caption="Fig 3. Chinook hatchery releases (bars; Table 2) relative to escapement and broodstock \ncollections (Figure 1). Release data from SEP and MRP, escapement data from NuSEDS.") +
  theme_bw() +
  theme(axis.text = element_text(colour="black", size=11),
        axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_text(face="bold", size=13),
...
```{r}
ggplot(sj.rel.bio, aes(x=RELEASE_YEAR, y=weight,  colour=RELEASE_STAGE_NAME, fill=RELEASE_STAGE_NAME)) +
  geom_point(size=4) +
...
```{r}
sj.MRPrr.sum <- sj.rel.rcvy %>% 
  group_by(`(RL) Tagcode`, `(RL) Num WithCWT Adclip`, `(RC) Recovery Year`, `(RC) Reporting Agency Code`, `(RC) Fishery PSC Code`, `(RC) MRP Area Name`, `(RC) Catch Region Name`, `(RC) PFMA SubArea Code`) %>%
